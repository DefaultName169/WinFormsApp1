...
namespace WebAPI.Models
{
    public class Document : DbContext
    {
        ...
        {
            return DBM.GetList("usp_Document_GetListByTicketID", new { TicketID }, out o);
        }
    ...
    [CustomValidation(typeof(WebAPI.Common.CustomValidation.CustomValidator), "PreventExpensiveMeetings")]
    public class DocumentInput : Audit
    {
        ...
        ///[StringLength(25, MinimumLength = 1, ErrorMessage = "Mã định danh văn bản, nội dung không được quá 25 kí tự")]
        ///
        [AssertThat("IsRegexMatch(DocCode, '^[a-zA-Z0-9.-]*$')", ErrorMessage = "Mã định danh văn bản, không được nhập ký tự có dấu và các ký tự đặc biệt")]
        public string DocCode { get; set; }
        ...
        public string CodeNumber { get; set; }
        [AssertThat("CheckTypeName == true", ErrorMessage = "Loại văn bản không tồn tại")]
        public int TypeName { get; set; }
        //[Required(ErrorMessage = "Hãy nhập Cơ quan - Tổ chức ban hành")]
        [AssertThat("CheckOrganName == true", ErrorMessage = "Cơ quan - Tổ chức ban hành không tồn tại")]
        public int OrganName { get; set; }
        ...
        [AssertThat("IsExist == true || (IsExist == false && (CongressMeeting == 0 || CongressMeeting == FileCongressMeeting))", ErrorMessage = "Kỳ họp bằng 0 hoặc bằng kì họp của hồ sơ")]
        public int CongressMeeting { get; set; }
        public int FileCongressMeeting { get; set; }
        [Range(0, 54, ErrorMessage = "Phiên họp, chỉ được nhập không quá 54")]
        [AssertThat("IsExist == true || (IsExist == false && (Meeting == 0 || Meeting == FileMeeting))", ErrorMessage = "Phiên họp bằng 0 hoặc bằng phiên họp của hồ sơ")]
        public int Meeting { get; set; }
        public int FileMeeting { get; set; }
        [AssertThat("IsExist == true || (IsExist == false && (NationalAssembly == 0 || NationalAssembly == FileNationalAssembly))", ErrorMessage = "Khóa bằng 0 hoặc bằng khóa của hồ sơ")]
        public int NationalAssembly { get; set; }
        ...
        public bool CheckTypeName
        {
            get
            {
                if (string.Equals(TypeName.GetTextFromSysCode("DOCUMENT_TYPE", ""), "")) return false;
                return true;
            }
        }

        /// <summary>
        /// Validate CheckOrganName
        /// </summary>
        [JsonIgnore]
        public bool CheckOrganName
        {
            get
            {
                if (string.Equals(OrganName.GetTextFromSysCode("ORGAN_NAME", ""), "")) return false;
                return true;
            }
        }

        /// <summary>
        /// Validate CheckAgencyCreate
        /// </summary>
        [JsonIgnore]
        public bool CheckAgencyCreate
        {
            get
            {
                if (string.Equals(AgencyCreate.GetTextFromSysCode("AGENCY_CREATE", ""), "")) return false;
                return true;
            }
        }

        /// <summary>
        /// Validate CheckFileNotation
        /// </summary>
        [JsonIgnore]
        public bool CheckFileNotation
        {
            get
            {
                string msg = CacheSystemKey.GetSystemKeyByParentAndChildValue("ORGAN_NAME", OrganName.ToString(), out SystemKey sysOrganName);
                if (msg.Length > 0 || sysOrganName.CodeKey == null) return false;
                msg = CacheSystemKey.GetSystemKeyByParentAndChildValue("AGENCY_CREATE", AgencyCreate.ToString(), out SystemKey sysAgencyCreate);
                if (msg.Length > 0 || sysAgencyCreate.CodeKey == null) return false;
                msg = CacheSystemKey.GetSystemKeyByParentAndChildValue("DOCUMENT_TYPE", TypeName.ToString().Trim(), out SystemKey sysTypeName);
                if (msg.Length > 0 || sysTypeName.CodeKey == null) return false;
                if (string.Equals(sysTypeName.CodeKey, "CV") && !string.Equals(FileNotation, string.Format("{0}/{1}-{2}", CodeNumber, sysOrganName.CodeKey, sysAgencyCreate.CodeKey))) return false;
                if (!string.Equals(sysTypeName.CodeKey, "CV") && !string.Equals(FileNotation, string.Format("{0}/{1}-{2}", CodeNumber, sysTypeName.CodeKey, sysOrganName.CodeKey))) return false;
                return true;
            }
        }
    }
